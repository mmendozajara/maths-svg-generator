...
An LLM-powered tool that takes a plain English description and produces a ready-to-paste `<DraftImage>` JSX snippet in ~10-30 seconds, with:

- SVG matching the Figma design system (colours, fonts, spacing)
- **Automatic image validation** (cutoff detection + mathematical accuracy via vision LLM)
- **Auto-regeneration** on validation failure (up to 3 retries with fix instructions)
- **Manual retry** button on result cards with remaining issues (sends fix instructions back to LLM)
...
|-----------|------|-------------|
| LLM Client | `llm_client.py` | Thread-safe OpenRouter API client with retry logic + vision support |
...
|-----------|------|-------------|
| Web UI | `app.py` + `templates/index.html` | Flask app with single/batch/JSX processor tabs, progress bar with ETA, cancel, batch downloads, manual retry. `--share` flag for ngrok, respects `PORT` env var for cloud deployment |
...
| Validation Prompt | `prompts/validate_image.md` | Strict vision validation checklist (cutoff + math accuracy) |
...
| Manual retry | Orange Retry button on cards with validation issues; sends fix instructions to LLM for regeneration with loading overlay and timer |
...
On failure, the validator returns structured fix instructions that are fed back to the generator LLM for auto-regeneration. If issues persist after all auto-retries, a manual Retry button appears on the result card.
...
| Sharing | Railway (permanent URL), ngrok (quick share), Docker |
...
| Metric | Value |
|--------|-------|
| Single diagram (generate + validate + upload) | ~10-20 seconds |
| With retries (1-3 validation retries) | ~20-60 seconds |
...
Permanent URL, auto-deploys on push:

1. [railway.app](https://railway.app/) → New Project → Deploy from GitHub
2. Add env vars: `OPENROUTER_API_KEY`, `IMGBB_API_KEY`
3. Auto-detects Dockerfile → deploys → permanent URL
